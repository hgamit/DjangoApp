...
# In alphabetical order by state, not by abbreviation #M263634281411 MN
STATE_FORMATS = {
    'AL': r'^[0-9]{7}$|^[a-z0-9]{6}$',
    'AK': r'^[0-9]{1,7}$',
...
def is_correct(dl_state, dl_number, allow_territories=False):
    """
    ...
    Test - is_correct('MN', 'M263634281411')
    ...
    if not dl_number:
        return False
        #raise Exception("No "+dl_number)

    if dl_state in STATE_FORMATS:
        regex = STATE_FORMATS[dl_state]
        is_correct = bool(re.match(regex, dl_number, re.IGNORECASE))
        #raise Exception(regex+" "+dl_state+dl_number)
        return is_correct
    elif allow_territories and dl_state in OTHER_STATES:
        return True
    else:
        #return False
        raise Exception(dl_state+dl_number+"Invalid state for drivers license number validation!")
